...
### 关于XREAL眼镜的状态回显命令的一些调查
当我们给眼镜发送命令时,他会返回一条命令包含完整的当前眼镜的工作状态等相关信息
已经调查到的一点点寄存器信息(一共有65个):
* 0x01  固定值为0xFD
* 0x18  按的哪个按键
    * 亮度模式 + 是0x27 - 是0x2a
    * 音量模式 + 是0x26 - 是0x2c或者0x29(不确定)
    * 电致变色按键 0x30
* 0x2e  每按键一次都会增加1
* 0x36  记录大小的值,按加减按键可以触发,当按键是在通过 "-" 按键切换到
    * 音量状态,他会是0-0x64的值
    * 亮度状态,他会是0-0x08的值
    * 按下电致变色按键 这个值是0-0x03(没有0x01)
